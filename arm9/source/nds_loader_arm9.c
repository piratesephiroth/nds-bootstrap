...
int runNds (const void* loader, u32 loaderSize, u32 cluster, u32 saveCluster, u32 dsiMode, u32 patchMpuRegion, u32 patchMpuSize, u32 consoleModel, u32 loadingScreen, u32 romread_LED, u32 gameSoftReset, bool initDisc, bool dldiPatchNds, int argc, const char** argv, u32* cheat_data)
{
	char* argStart;
	u16* argData;
	u16 argTempVal = 0;
...
int runNdsFile (const char* filename, const char* savename, int dsiMode, int patchMpuRegion, int patchMpuSize, int consoleModel, int loadingScreen, int romread_LED, int gameSoftReset, int argc, const char** argv, u32* cheat_data) {
	struct stat st;
	struct stat stSav;
	u32 clusterSav = 0;
	...
	return runNds (load_bin, load_bin_size, st.st_ino, clusterSav, dsiMode, patchMpuRegion, patchMpuSize, consoleModel, loadingScreen, romread_LED, gameSoftReset, true, true, argc, argv, cheat_data);
}
