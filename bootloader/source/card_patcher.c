...
u32 a9cardReadSignatureThumb5[2]    = {0x040001A4, 0xFFFFFE00};
u32 cardReadStartSignature5[1] = {0xE92D4FF8};
u32 cardReadStartSignatureThumb5[1] = {0xB086B5F8};

u32 a9cardIdSignature[2]      = {0x04100010,0xE92D4038};
u32 a9cardIdSignatureThumb[4]    = {0x02FFFAE0, 0xF8FFFFFF, 0x040001A4, 0x04100010};
...
u32 cardReadDmaStartSignatureThumb[1]   = {0xB082B5F8};
...
u32 generateA7InstrThumb(int arg1, int arg2) {
    ...
    u16 firstInstr = (( offset >> 12) & 0x7FF) | 0xF000;
...
u32 patchCardNdsArm9 (const tNDSHeader* ndsHeader, u32* cardEngineLocation, module_params_t* moduleParams, u32 patchMpuRegion, u32 patchMpuSize) {
	...
	if (usesThumb) {
		cardReadStartOffset =   
			getOffset((u32*)cardReadEndOffset, -0xD0,
	...
	if(!cardIdEndOffset && usesThumb){
		cardIdEndOffset =  
        getOffset((u32*)ndsHeader->arm9destination, ndsHeader->arm9binarySize,
              (u32*)a9cardIdSignatureThumb, 4, 1);
	}
    if (!cardIdEndOffset) {
...
u32 savePatchV5 (const tNDSHeader* ndsHeader, u32* cardEngineLocation, module_params_t* moduleParams, u32 saveFileCluster) {
	...
	if (usesThumb) {
		...
		u32* eepromRead = (u32*) (JumpTableFunc + 0x8);
		dbg_printf("Eeprom read:\t");
		dbg_hexa((u32)eepromRead);
		dbg_printf("\n");
		srcAddr = JumpTableFunc + 0x8  - vAddrOfRelocSrc + 0x37F8000 ;
		u32 patchRead = generateA7InstrThumb(srcAddr,
			arm7FunctionThumb[5]+1);
		*eepromRead=patchRead;
		...
		u32 patchWrite = generateA7InstrThumb(srcAddr,
			arm7FunctionThumb[3]+1);
		*eepromPageWrite=patchWrite;

		u32* eepromPageProg = (u32*) (JumpTableFunc + 0x24);
		dbg_printf("Eeprom page prog:\t");
		dbg_hexa((u32)eepromPageProg);
		dbg_printf("\n");
		srcAddr = JumpTableFunc + 0x24 - vAddrOfRelocSrc + 0x37F8000 ;
		u32 patchProg = generateA7InstrThumb(srcAddr,
			arm7FunctionThumb[4]+1);
		*eepromPageProg=patchProg;

		u32* eepromPageVerify = (u32*) (JumpTableFunc + 0x32);
		dbg_printf("Eeprom verify:\t");
		dbg_hexa((u32)eepromPageVerify);
		dbg_printf("\n");
		srcAddr =  JumpTableFunc + 0x32 - vAddrOfRelocSrc + 0x37F8000 ;
		u32 patchVerify = generateA7InstrThumb(srcAddr,
			arm7FunctionThumb[2]+1);
		*eepromPageVerify=patchVerify;


		u32* eepromPageErase = (u32*) (JumpTableFunc + 0x3E);
